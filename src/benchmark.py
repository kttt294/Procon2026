    ...
    for game_idx in range(n_games):
        ...
        for strat_name in strategies:
            ...
            all_results[strat_name].append(
                GameResult(strat_name, compute_score(state), elapsed_ms)
            )
...
def print_results(results: Dict[str, BenchmarkResult]) -> None:
    header = f"{'Strategy':<12} {'Avg Series':>10} {'Avg Daily':>10} {'Avg Udon':>10} {'Avg ms':>8} {'Win Rate':>10}"
    print()
    print(header)
    print("-" * len(header))
    for r in results.values():
        ...
        print(
            ...
            f"{r.avg_time_ms:>8.1f} {win_str:>10}"
        )
    print()
